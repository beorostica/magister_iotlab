...
var fs = require('fs');
fs.writeFile('commands.sh','#!/bin/bash\n\n');
...
function runSingleCommandWithWait() {
	...
	if (dataAuthentication == "Written") {
            ...
            if (expStatus == "Running") {
                ...
                for (i = 0; i < N; i++) {
                    if (nodeSite[i] == "grenoble") {
                        nodeIDsGrenoble = nodeIDsGrenoble + nodeID[i] + '+';
                    }
                    if (nodeSite[i] == "saclay") {
                        nodeIDsSaclay = nodeIDsSaclay + nodeID[i] + '+';
                    }
                    if (nodeSite[i] == "strasbourg") {
                        nodeIDsStrasbourg = nodeIDsStrasbourg + nodeID[i] + '+';
                    }
                }
                ...
                fs.appendFile('commands.sh','iotlab-ssh --verbose run-cmd "cd A8 && node IoTLab_protocol.js" -l grenoble,a8,' + nodeIDsGrenoble + ' &\n');
                fs.appendFile('commands.sh','iotlab-ssh --verbose run-cmd "cd A8 && node IoTLab_protocol.js" -l saclay,a8,' + nodeIDsSaclay + ' &\n');
                fs.appendFile('commands.sh','iotlab-ssh --verbose run-cmd "cd A8 && node IoTLab_protocol.js" -l strasbourg,a8,' + nodeIDsStrasbourg + ' &\n');
                fs.appendFile('commands.sh','sleep 30\n');
                fs.appendFile('commands.sh','iotlab-ssh --verbose run-cmd "cd A8 && node IoTLab_start_signal.js" --frontend\n');
                fs.appendFile('commands.sh','sleep 2m\n');
                fs.appendFile('commands.sh','iotlab-ssh --verbose run-cmd "cd A8 && node IoTLab_stop_signal.js" --frontend\n');
            }
        }
        yield cmd.run("chmod 777 commands.sh");
        yield cmd.run('./commands.sh');
        console.log("");
